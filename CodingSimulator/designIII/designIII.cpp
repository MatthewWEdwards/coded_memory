...
int mem_stall;
int parity_hit = 0;
bool parity_stall[NUM_PARITY_BANKS];
...
void input_controller(vector<transLevelRequest> request_queue[]) {
	...
	for(int i = 0; i < NUM_TRACES; i++) {
		...
		/*for(int y = 0; y < NUM_BANKS; y++) {
			for(int z = 0; z < bankWriteQueue[y].size(); z++) {
				if(request_queue[i].size() > 0) {
					if(bankWriteQueue[y][z].address == request_queue[i][0].address && request_queue[i][0].time <= current_time) {
						reads_served_from_write++;
...
void access_scheduler() {
	...
	for(int mainDataBank = 0; mainDataBank < NUM_BANKS; mainDataBank++) {
		...
		if((dataBankStatus[mainDataBank] == BANK_FREE) && bankWriteQueue[mainDataBank].size() < WR_QUEUE_BUILDUP && bankReadQueue[mainDataBank].size() != 0 && (current_time % MEM_DELAY) == 0) {
			 ...
			 * parity banks */
			if(bankReadQueue[mainDataBank][0].critical == true) {
				read_cr_word_latency += (current_time) - bankReadQueue[mainDataBank][0].time;
				fprintf(dump, "Delay: %d\t Address: %d 	Time: %d\n", current_time - bankReadQueue[mainDataBank][0].time, bankReadQueue[mainDataBank][0].address, current_time);
				//fprintf(dump, "Delay: %d	Address: %d	Time: %d\n", current_time - bankReadQueue[mainDataBank][0].time, bankReadQueue[mainDataBank][0].address, bankReadQueue[mainDataBank][0].time);
			}
			past_requests.push_back(bankReadQueue[mainDataBank][0]);
			int region = (bankReadQueue[mainDataBank][0].address - lowAddress)/region_size;
			if(region >= NUM_REGIONS) {
				cout << region << " " << lowAddress << " " << bankReadQueue[mainDataBank][0].address << " " << highAddress << endl;
				exit(0);
			}
			previously_read[region].push_back(bankReadQueue[mainDataBank][0].address); //Record that the memory location is now coded
			serve_request(bankReadQueue[mainDataBank][0]);
			bankReadQueue[mainDataBank].erase(bankReadQueue[mainDataBank].begin());
		}
		else if((dataBankStatus[mainDataBank] == BANK_FREE) && bankWriteQueue[mainDataBank].size() != 0 && (current_time % MEM_DELAY) == 0) {
			/* First serve the request in the data bank */
			if(bankWriteQueue[mainDataBank][0].critical == true) {
				write_cr_word_latency += (current_time) - bankWriteQueue[mainDataBank][0].time;
			}
			serve_request(bankWriteQueue[mainDataBank][0]);
			bankWriteQueue[mainDataBank][0].inParity = false; //Mark that the write was not written to parity
			overwritten_parity_rows.push_back(bankWriteQueue[mainDataBank][0]);
			bankWriteQueue[mainDataBank].erase(bankWriteQueue[mainDataBank].begin());	

			/* If another request is waiting behind it, try to use the parity banks */
			if(bankWriteQueue[mainDataBank].size() != 0) {
				...
				if(parity_stall[parity_bank_num] == BANK_FREE) {
					parity_stall[parity_bank_num] = BANK_BUSY; //Mark the bank as busy
					...
					serve_request(bankWriteQueue[mainDataBank][0]);
					bankWriteQueue[mainDataBank][0].inParity = true; //Mark that the write was written to parity
					bankWriteQueue[mainDataBank][0].parityNumber = parity_bank_num;
					overwritten_parity_rows.push_back(bankWriteQueue[mainDataBank][0]);
					bankWriteQueue[mainDataBank].erase(bankWriteQueue[mainDataBank].begin());
				}
			}
...
bool check_recode() {
	...
	for(int i = 0; i < pending_parity_writes.size(); i++) {
		...
		for(int n = 0; n < 3; n++)
			parity_stall[parity_bitmap[bank % 4][n]] = BANK_BUSY;
	}
			...
			for(int i = 0; i < overwritten_parity_rows.size(); i++) {
				if(overwritten_parity_rows[i].address/8 == row) {
					...
					if(overwritten_parity_rows[i].inParity == true) {
						parity_stall[overwritten_parity_rows[i].parityNumber] = BANK_BUSY;
					}
					else
