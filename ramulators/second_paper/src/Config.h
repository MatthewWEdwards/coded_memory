...
public:
    ...
    int get_parity_architecture() const {return parity_architecture;}
    int get_rob_length() const {return rob_length;}
    ...
    bool print_bank_queues() const {
      // the default value is false
      if (options.find("print_bank_queues") != options.end()) {
        if ((options.find("print_bank_queues"))->second == "on") {
          return true;
        }
        return false;
      }
