...
void core_arbiter::action(void){
	...
	{
		//cout << arbiter_read_out[k]->s<< endl;
		//cout << arbiter_read_out[k]->num_free()<<endl;
		if (arbiter_read_out[k]->num_free()==0)
		{
			flag = false;
			//cout << k << endl;
			break;
		}
		//cout << arbiter_write_out[k]->num_free() << endl;
		if (arbiter_write_out[k]->num_free() == 0)
		{
			flag = false;
			break;
		}
	...
	case 0: 	//size is the size of word here usually is 256bits ==32B	
			for (int i = 0; i < info.length; i++,addr++){			//INCR
				...
				int channel = addr%8;				//suppose it 5-7-4-3-5
													//row column bank channel word
				queue_request channel_request(addr, info.id, info.core_number, info.time);
			
				if (info.read==true)
				{	
					if (i==0)
					{
						channel_request.critical = true;
					...
					arbiter_read_out[channel]->write(channel_request);
					//assert(false);
				...
				else
				{
					...
					arbiter_write_out[channel]->write(channel_request);
					//assert(false);
				}				
			}	
		break;
	case 1:	
		...
		{													// WRAP	
			for (int i = 0; i < info.length; i++){		
				...
				if (info.read == true)
				{
					if (i == 0){ channel_request.critical = true; }//the first read is critical read
							//cout << "read request: bank" << bank << "	address:" << std::hex << bank_request.addr << "	id" << info.id << "	core_num" << info.core_number<<"critical read or not" << bank_request.critical  << endl;
					arbiter_read_out[channel]->write(channel_request);
