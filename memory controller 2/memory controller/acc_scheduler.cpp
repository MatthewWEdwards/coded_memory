...
int dataBankStatus[NUM_BANKS];		//free or busy
int parityBankStatus[NUM_PARITY_BANKS];

int codingstatusmap[NUM_ROWS][NUM_PARITY_BANKS];


long double tollcritical_time;
int tollcritical_count;
long double tolltransactional_time;
int tolltransactional_count;

long double localcritical_time;
int localcritical_count;
long double localtransactional_time;
int localtransactional_count;

long double localwrite_time;
int localwrite_count;
long double tollwrite_time;
int tollwrite_count;
...
int trans2list(sc_fifo_in<queue_request>* inport, deque<list_request>** list, int cycle_priority, sc_fifo_in<queue_request>* tollread_bankin);
...
int decisioner(int* tr, int* tw, int* lr, int* lw){
	int d1 = *tr >= *tw ? 1 : 0;
	int d2 = *lr > 2**lw ? 1 : 0;
	int res = d1 * 2 + d2;
	switch (res)
...
void ACCSDULER::scheduler_action(void){
	...
	/*
	read request from core arbiter and push it into the R/W list
	when the list is not full
	*/
	//cout << "scheduler action" << endl;
	...
	tollread_prio = trans2list(tollread_bankin, tollreadlist, tollread_prio, tollread_bankin);
	tollwrite_prio = trans2list(tollwrite_bankin, tollwritelist, tollwrite_prio, tollread_bankin);
	localread_prio = trans2list(localread_bankin, localreadlist, localread_prio, tollread_bankin);
	localwrite_prio = trans2list(localwrite_bankin, localwritelist, localwrite_prio, tollread_bankin);
	/*myfile << "tollread_prio" << tollread_prio << endl;
	myfile << "tollwrite_prio" << tollwrite_prio << endl;
	myfile << "localread_prio" << localread_prio << endl;
	myfile << "localwrite_prio" << localwrite_prio << endl;*/
	for (int k = 0; k < NUM_BANKS; k++)
	{
		if (dataBankStatus[k] == BANK_BUSY) {
			bankbusyflag = true;
			break;
		}
	}
	...
	{
		if (parityBankStatus[k] == BANK_BUSY) {
			bankbusyflag = true;
			break;
		}
	}
	if (!bankbusyflag)
	{
		next_cycle = decisioner(&tollread_prio, &tollwrite_prio, &localread_prio, &localwrite_prio);

		//cout << "current cycle" << current_cycle << endl;
		switch (current_cycle)
		{
		case tollread:tollreadserve();
			break;
		case tollwrite:tollwriteserve();
			break;
		case localread:localreadserve();
			break;
		case localwrite:localwriteserve();
			break;
	...
	//cout << sc_time_stamp() << endl;
	//assert(sc_time_stamp() < sc_time(20000, SC_NS));
	//myfile.close();
	free_all_banks();
	ACCSDULER::served_request->clear();		//clear the served request
}
...
int trans2list(sc_fifo_in<queue_request>* inport, deque<list_request>** list, int cycle_priority, sc_fifo_in<queue_request>* tollread_bankin){
	//inport should one of tollread_bankin localread_bankin tollwrite_bankin localwrite_bankin
	//list should be one of tollreadlist tollwritelist  localreadlist localwritelist
	bool successread;
	queue_request readinfo;
	int total=0;
	...
	//
	for(int i=0;i<NUM_BANKS;i++){
		if (list[i]->size()==listmaxsize)
		{
			cycle_priority+=2;
		}
		
		assert(list[i]->size() <= listmaxsize);
		while(list[i]->size()<listmaxsize)		
		{
			successread = inport[i]->nb_read(readinfo);
			if (successread)
			{
				list_request listinfo(readinfo);
				//if (!list[i]->empty()){
				//	if ((list[i]->back().time)-listinfo.time>200)		//problem is here!!!!!!!!!!!!!!!!!!!!!!!
				//	{
				//		cout << "list[i]" << '\t' << list[i]->back() << '\t' << listinfo.time;
				//	assert(false);
				//	}
				//}
				//dump file
				/*if (inport == tollread_bankin){
					record_bankout(i, readinfo);
				}*/
				list[i]->push_back(listinfo);
			}
...
//set status of all banks to free, when all actions have been taken, cycle ends
void free_all_banks(void){
	static int databanks_frequency[NUM_BANKS];
	static int paritybank_frequency[NUM_PARITY_BANKS];

	for (int i = 0; i < NUM_BANKS; i++)
	{
		if (dataBankStatus[i]==BANK_BUSY){
			if (bank_frequency==++databanks_frequency[i])
			{
				dataBankStatus[i] = BANK_FREE;
				databanks_frequency[i] = 0;
			}
		}
	}
	...
	{
		if (parityBankStatus[i] == BANK_BUSY){
			if (bank_frequency == ++paritybank_frequency[i])
			{
				parityBankStatus[i] = BANK_FREE;
				paritybank_frequency[i] = 0;
			}
		}
	}
	return;
